...
from .constants import RELATIONSHIP_OPTIONS, NICKNAME_OPTIONS, PERSONALITY_OPTIONS, is_custom_value, \
    FREE_RELATIONSHIP_OPTIONS, FREE_NICKNAME_OPTIONS, FREE_PERSONALITY_OPTIONS
...
class UsersAssistantTemplatesViewSet(ListModelMixin,
    ...
    @action(detail=False, methods=['post'])
    def restore_default(self, request):
        """
        将用户的助手模板恢复为系统默认设置
        """
        user_id = request.remote_user.get('id')
        ...
        try:
            ...
            # 生成提示词
            prompt = self.generate_prompt(default_template.prompt_template, config)
